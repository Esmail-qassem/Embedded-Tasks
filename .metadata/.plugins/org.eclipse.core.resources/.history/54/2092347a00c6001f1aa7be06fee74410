...
typedef struct
{
	...
	u8 FOC1B :1; /*Force Output Compare for Channel B*/
	u8 FOC1A :1; /*Force Output Compare for Channel A*/
	u8 COM1BX:2; /*compare output mode for channel B*/
	u8 COM1AX:2; /*compare output mode for channel A*/
}TCCR1A;

typedef struct
{
	u8 CS1X   :3;  /*Clock Select bits*/
	u8 WGM1XX :2;  /* Waveform Generation Mode */
	u8        :1;  /*Reserved*/
	u8 ICES1  :1;  /*Input Capture Edge Select*/
	u8 ICNC1  :1;  /* Waveform Generation Mode */
}TCCR1B;
...
{
	u8       :2;
	u8 TOIE1 :1; /*Timer/Counter1, Overflow Interrupt Enable*/
	u8 OCIE1B:1; /*Timer/Counter1, Output Compare B Match Interrupt Enable*/
	u8 OCIE1A:1;/*Timer/Counter1, Output Compare A Match Interrupt Enable*/
	u8 TICIE1:1;/*Timer/Counter1, Input Capture Interrupt Enable*/
	u8       :2;
}Timer1_TIMSK;
...
{
	u8       :2;
	u8 TOV1  :1; /*Timer/Counter1, Overflow Flag*/
	u8 OCF1B :1; /*Timer/Counter1, Output Compare B Match Flag*/
	u8 OCF1A: 1; /*Timer/Counter1, Output Compare A Match Flag*/
	u8 ICF1  :1; /*Timer/Counter1, Input Capture Flag*/
	u8       :2;
}Timer1_TIFR;
