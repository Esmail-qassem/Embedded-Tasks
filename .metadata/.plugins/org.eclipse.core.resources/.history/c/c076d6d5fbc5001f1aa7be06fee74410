...
status_t DIO_SetPinVal(Port_t Copy_Port, Pin_t Copy_Pin, u8 Copy_u8Value) {
	status_t Local_u8ErrorState = DIO_ok;
...
status_t DIO_SetPortDir(Port_t Copy_Port, u8 Copy_u8Direction) {
	status_t Local_u8ErrorState = DIO_ok;
	if ((Copy_Port > DIO_PORTD) || (Copy_Port < DIO_PORTA)) {
		Local_u8ErrorState = DIO_DirErr;
	} else {
		switch (Copy_Port) {
		case DIO_PORTA:
...
status_t DIO_GetPinVal(Port_t Copy_Port, Pin_t Copy_Pin, u8* Copy_pvValue)
{
	status_t Local_u8ErrorState = DIO_ok;
...
u8 DIO_GetPortVal(Port_t Copy_Port,u8* Copy_pvValue)
{
